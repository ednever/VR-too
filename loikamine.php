...
if(!empty($_REQUEST["korras_id"])){
    $kask=$yhendus->prepare("UPDATE rulood SET riievalmis = 1 WHERE id = ?");
    $kask->bind_param("i", $_REQUEST["korras_id"]);
    $kask->execute();
}
if(!empty($_REQUEST["vigane_id"])){
    $kask=$yhendus->prepare("UPDATE rulood SET riievalmis = 2 WHERE id = ?");
    $kask->bind_param("i", $_REQUEST["vigane_id"]);
    $kask->execute();
}
$kask=$yhendus->prepare("SELECT id, mustinr FROM rulood WHERE riievalmis = -1 OR riievalmis = 2");
